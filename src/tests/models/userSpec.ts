...
describe('Testing user Model', () => {
  ...
  it('index method should return a list of users', async () => {
    ...
    expect(result[0].userName).toEqual('Patty');
    expect(result[0].firstName).toEqual('TCHINGUÉ');
    expect(result[0].lastName).toEqual('Patricia');
  });

  it('show method should return an user', async () => {
    const result:User = await user.show(u.id as number);
    expect(result.userName).toEqual('Patty');
    expect(result.firstName).toEqual('TCHINGUÉ');
    expect(result.lastName).toEqual('Patricia');
  });
  ...
  it('delete method should remove the user', async () => {
     await user.delete(u.id  as number);
